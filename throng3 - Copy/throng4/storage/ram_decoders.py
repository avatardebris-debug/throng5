...
class MontezumaDecoder(BaseDecoder):
    ...
    # Skull direction byte: observed values 93 (heading right) and 152 (heading left).
    # This is likely a sprite state/direction flag, not raw pixel x.
    # The skull patrols the bottom floor (y≈148) between the rope and the right wall.
    # byte 40 = 93  → skull moving right (toward right wall)
    # byte 40 = 152 → skull moving left  (toward rope side)
    # Confidence: medium — derived from RAM oscillation analysis, not manual verification.
    SKULL_BYTE        = 40
    SKULL_LEFT_VAL    = 152   # heading left
    SKULL_RIGHT_VAL   = 93    # heading right
    # byte 24: skull phase / position along patrol (0-129 range, increments each step)
    # Use as a continuous proxy for skull x until a precise byte is found.
    SKULL_PHASE_BYTE  = 24
    ...
    def decode(self, ram: np.ndarray) -> dict[str, Any]:
        ...
        # IMPORTANT: At game start, RAM[56] may not be 0 with all ALE seeds.
        # We check BOTH bytes: key is held only if RAM[56]==0xFF AND bit1 of RAM[65] is set.
        # This prevents false positives if one byte happens to be 0xFF at start.
        key_raw    = int(ram[56])
        items_mask = int(ram[65])           # secondary: bit1=key (value 2)
        items_raw  = items_mask             # expose for debugging

        # Robust key detection: RAM[56]==0xFF (primary) AND items_mask bit1 (secondary)
        key_held_primary   = (key_raw == 0xFF)          # 255
        key_held_secondary = bool(items_mask & 0x02)    # bit 1 of items byte
        key_bit = key_held_primary and key_held_secondary
        ...
        # Skull tracking (bottom floor, y≈148)
        # byte 40: direction flag (93=right, 152=left). Confidence: medium.
        skull_dir_raw  = int(ram[self.SKULL_BYTE])
        skull_left     = (skull_dir_raw == self.SKULL_LEFT_VAL)
        skull_phase    = int(ram[self.SKULL_PHASE_BYTE])  # 0-129, proxy for position
        skull_phase_norm = round(skull_phase / 129.0, 4)
        ...
        return {
            ...
            "items_raw":      items_raw,
            "score":          score,
            # Skull tracking (medium confidence — calibrated from RAM oscillation)
            "skull_dir_raw":   skull_dir_raw,
            "skull_heading_left": skull_left,    # True = skull moving toward rope
            "skull_phase":     skull_phase,       # 0-129 patrol phase proxy
            "skull_phase_norm": skull_phase_norm,
            ...
            "_confidence":    {
                ...
                "key":       "calibrated",
                "score":     "estimated",
                "skull":     "medium",   # direction flag calibrated; x-phase proxy
            },
        }
