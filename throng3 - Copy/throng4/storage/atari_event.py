...
class AtariEventLogger:
    ...
    def log_step(
        ...
        ram_obs: np.ndarray | list[int] | None = None,
    ) -> dict[str, Any]:
        """
        ...
        ram_obs:  Optional 128-byte ALE RAM observation. When provided,
                  stored as a compact int list under "ram" in the JSONL.
                  Enables discover_subgoals.py to run reward-correlation
                  analysis without a separate RAM log file.
                  Pass None (default) to omit — backward compatible.
        ...
        # Include RAM snapshot when provided (opt-in — keeps file size manageable
        # for callers that don't need autonomous subgoal discovery)
        if ram_obs is not None:
            event["ram"] = [int(b) for b in ram_obs[:128]]
