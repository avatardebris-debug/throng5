...
class PortableNNAgent:
    ...
    def get_mcts_prior(
        ...
        """
        Produce a prior probability distribution over actions for MCTS.

        Args:
            features_list: List of feature vectors, one for each valid action.
            temperature:   Softmax temperature. Lower = more greedy toward max Q.

        Returns:
            np.ndarray of shape (len(features_list),) summing to 1.0
        """
        if not features_list:
            return np.array([])
            
        X = np.stack(features_list)
        q_vals = self.forward_batch(X)
        
        # Softmax over Q values
        if temperature <= 0.0:
            probs = np.zeros_like(q_vals)
            probs[np.argmax(q_vals)] = 1.0
            return probs
            
        # Scale by temperature and shift for numerical stability
        scaled_q = q_vals / temperature
