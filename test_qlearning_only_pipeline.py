"""
Q-Learning ONLY in Meta^N Pipeline (STDP/Hebbian DISABLED)

This test validates the hypothesis that mixed learning causes interference.
Expected result: ~100% success (matching standalone Q-learning)
"""
...
print("="*60)
print("Q-Learning ONLY in Meta^N Pipeline")
print("="*60)
print("STDP/Hebbian DISABLED - Testing Q-learning isolation")
print()

# Create pipeline with Q-learning ONLY (disable bio-inspired)
synapse_config = SynapseConfig(
    ...
    # CRITICAL: Disable STDP and Hebbian
    default_rule='none',  # No bio-inspired learning
)
...
print(f"  Q-learning: ENABLED")
print(f"  STDP: DISABLED")
print(f"  Hebbian: DISABLED")
print(f"  Default rule: {synapse_config.default_rule}")
print()

env = GridWorldAdapter()

# ============================================================
# PHASE 1: CURRICULUM LEARNING (5 demonstrations)
# ============================================================
print("="*60)
...
print(f"\nExpected results:")
print(f"  Standalone Q-learning:     100% success")
print(f"  Mixed learning (existing): 29.5% success (degrading)")
print(f"  Q-only in pipeline:        ~100% success (if hypothesis correct)")

print(f"\nActual result:")
print(f"  Q-only in pipeline:        {successes/95:.1%} success")

if successes / 95 > 0.9:
    print(f"\n✓ HYPOTHESIS CONFIRMED!")
    print(f"  Q-learning works perfectly when isolated ({successes/95:.1%})")
    print(f"  Mixed learning interference is the root cause")
    print(f"  → Regional separation (Throng3.5) is the right path")
elif successes / 95 > 0.5:
    print(f"\n⚠ PARTIAL CONFIRMATION")
    print(f"  Better than mixed ({successes/95:.1%} vs 29.5%)")
    print(f"  But not as good as standalone (100%)")
    print(f"  → May be other issues beyond mixed learning")
else:
    print(f"\n✗ HYPOTHESIS REJECTED")
    print(f"  Q-only still fails ({successes/95:.1%})")
    print(f"  → Problem is NOT mixed learning interference")
    print(f"  → Need to debug Q-learning integration further")
